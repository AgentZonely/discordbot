...
module.exports.run = async (bot, message, args) => {
    ...
    if(!args[0].endsWith("d") && !args[0].endsWith("h") && !args[0].endsWith("m"));
      message.channel.send(`**The time needs to be specified in **days (d), hours (h), or minutes (m)**`);
      if (isNaN(args[0])) return message.channel.send(`It must be a **number** you know that?`);
